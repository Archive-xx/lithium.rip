...
|________/|__/   \___/  |__/  |__/|__/ \______/ |__/ |__/ |__/      |________/|__/   \___/   \_______/                                                                                                     
    ...
    lith::info::stage = 3;
    int result = lith::auth::authenticate(hwid, random_string(64));
    lith::info::stage = 4;

    if (result == lith::auth::authenticated) {

        std::cout << xors("Authenticated.") << std::endl;
        lith::info::verified = true;
        std::this_thread::sleep_for(std::chrono::seconds(3));
    }
    else if (result == lith::auth::not_authorized) {

        std::cout << xors("Not authorized, HWID copied to clipboard.") << std::endl;
        copy_to_clipboard(0, hwid);
        std::this_thread::sleep_for(std::chrono::seconds(3));
        std::exit(EXIT_SUCCESS);
    }
    else if (result == lith::auth::outdated) {

        std::cout << xors("Outdated version, please redownload.") << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(3));
        std::exit(EXIT_SUCCESS);
    }
    else if (result == lith::auth::error) {

        std::string cmd = xors("/C echo Error - Lithium Lite v") + std::string(lith::info::version) + xors(" & echo. & echo An error occurred while authenticating, please contact support. ") + std::string("Stage: " + std::to_string(lith::info::stage)) + xors("& echo. & pause");
